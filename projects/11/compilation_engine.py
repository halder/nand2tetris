# TODO: input: tokenizer output, output: VMWriter code.
#       instead of XML code, write actual VM code!
#import symbol_table
...
class Parser:
    ...
    def compile_class_var_dec(self) -> None:
        ...
        next_type, next_value = self.peek() # int/char/boolean/identifier
        ...
        next_type, next_value = self.peek() # variable name
        if self.is_identifier(next_type):
            self.advance()
        ...
        #self.file.write(f"symbol table entry for most recent var: name: {var_name}, type: {var_type}, kind: {var_kind}, index: {self.symbol_table.index_of('class', var_name)}\n")

        next_type, next_value = self.peek()
        while self.is_literal(next_type, next_value, ","):
            self.advance()

            next_type, next_value = self.peek() # more variable names
            if self.is_identifier(next_type):
                self.advance()
                #var_name = self.current_value
                #self.symbol_table.define(var_name, var_type, var_kind)
    ...
    def compile_subroutine(self) -> None:
        ...
        # self.current: constructor/function/method

        next_type, next_value = self.peek()
        if self.is_type(next_type, next_value) or (self.is_keyword(next_type) and next_value == "void"): # function return type
            self.advance()
        else:
            raise ParserError(f"expected 'void', 'int', 'char', 'boolean' or identifier after {self.current_value}, found '{next_type}': '{next_value}'")
        
        next_type, next_value = self.peek()
        if self.is_identifier(next_type): # function name
            self.advance()
        else:
            raise ParserError(f"expected identifier after '{self.current_type}': '{self.current_value}', found '{next_type}': '{next_value}'")
    ...
    def compile_parameter_list(self) -> None:
        ...
        #var_kind = "argument"
        #var_type = self.current_value
        # self.current: arg1 type

        next_type, next_value = self.peek() # arg1 name
        if self.is_identifier(next_type):
            self.advance()
            #var_name = self.current_value
            #self.symbol_table.define(var_name, var_type, var_kind)
        ...
        while self.is_literal(next_type, next_value, ","):
            ...
            next_type, next_value = self.peek() # arg+ type
            if self.is_type(next_type, next_value):
                self.advance()
                #var_type = self.current_value
            else:
            ...
            next_type, next_value = self.peek() # arg+ name
            if self.is_identifier(next_type):
                self.advance()
                #var_name = self.current_value
                #self.symbol_table.define(var_name, var_type, var_kind)
